...
const CAPABILITY_MESSAGES={
  en:{capability_storage:"Can store the extension's own settings and local data.",capability_tabs:"Can inspect metadata about open tabs.",capability_cookies:"Can access cookies on allowed sites, including data used for sessions and tracking.",capability_webRequest:"Can observe requests made by pages.",capability_webRequestBlocking:"Can block or modify certain requests before they complete.",capability__all_urls_:"Can operate across a broad range of sites.",capability_debugger:"Can use advanced debugging features to inspect pages.",capability_nativeMessaging:"Can communicate with an application installed on the computer."},
  es:{capability_storage:"Puede guardar configuraciones y datos locales de la propia extensión.",capability_tabs:"Puede consultar metadatos de las pestañas abiertas.",capability_cookies:"Puede acceder a cookies de sitios permitidos, incluidos datos usados en sesiones y seguimiento.",capability_webRequest:"Puede observar solicitudes realizadas por las páginas.",capability_webRequestBlocking:"Puede bloquear o modificar determinadas solicitudes antes de que finalicen.",capability__all_urls_:"Puede actuar en una gran cantidad de sitios.",capability_debugger:"Puede usar funciones avanzadas de depuración para inspeccionar páginas.",capability_nativeMessaging:"Puede comunicarse con una aplicación instalada en el equipo."}
};
...
function reasonText(reason) {
  ...
  if (currentLanguage !== "pt-BR") {
    ...
    else if (String(reason.id).startsWith("capability_")) message = CAPABILITY_MESSAGES[currentLanguage]?.[reason.id] || (currentLanguage === "es" ? "Esta capacidad técnica amplía el acceso de la extensión." : "This technical capability expands the extension's access.");
    ...
    else if (reason.id === "context_compatible_permissions") message = currentLanguage === "es" ? "Los permisos principales son compatibles con la función detectada." : "The main permissions are compatible with the detected function.";
    else if (reason.id === "context_unknown_category") message = currentLanguage === "es" ? "No se pudo determinar con confianza la categoría funcional de la extensión." : "The extension's functional category could not be determined with confidence.";
...
function renderExtensionRiskDetail(extension) {
  ...
  const confidence=element("section","risk-score-card");confidence.append(element("span","",t("confidence")),element("strong","",`${risk.confidence||0}%`),element("p","",`${t("detectedCategory")}: ${["other","unknown"].includes(risk.category?.id)?t("unknownCategory"):risk.category?.id||t("unknownCategory")}`));scores.append(confidence);panel.append(scores);
