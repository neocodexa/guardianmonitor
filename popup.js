...
Object.assign(I18N["pt-BR"],{guarded:"risco de atenção",critical:"risco crítico",developedBy:"Dev pela"});Object.assign(I18N.en,{guarded:"guarded risk",critical:"critical risk",developedBy:"By"});Object.assign(I18N.es,{guarded:"riesgo de atención",critical:"riesgo crítico",developedBy:"Por"});
...
function renderEvent(event) {
  ...
  const riskLabel = t(["low", "guarded", "medium", "high", "critical"].includes(event.riskLevel) ? event.riskLevel : "low");
  const detail = event.type === "download_scan"
    ? `${files[0]?.name || t("download")} · ${event.riskScore ?? 0}/100 · ${riskLabel} · Chromium: ${event.danger || t("unknown")}`
    : event.type === "credential_form_risk"
      ? `${event.riskScore ?? 0}/100 · ${riskLabel} · ${t("destination")}: ${event.targetDomain || t("unknown")} · ${t("noPassword")}`
